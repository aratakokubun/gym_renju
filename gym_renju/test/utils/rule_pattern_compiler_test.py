...
class RulePatternCompilerTest(ut.TestCase):
  ...
  def test_go_ren(self, player_color: PlayerColor, player_latest: PlayerLatest, pattern: str, match_nums: int):
    patterns_dict = rpc.pcompile(player_color, player_latest)
    re_pattern = patterns_dict[rpc.SpecialPatterns.GO_REN]
  ...
  def test_tyo_ren(self, player_color: PlayerColor, player_latest: PlayerLatest, pattern: str, match_nums: int):
    patterns_dict = rpc.pcompile(player_color, player_latest)
    re_pattern = patterns_dict[rpc.SpecialPatterns.TYO_REN]
    matched = re.findall(re_pattern, pattern)
    self.assertEqual(len(matched), match_nums)
  ...
  def test_tasshi(self, player_color: PlayerColor, player_latest: PlayerLatest, pattern: str, match_nums: int):
    patterns_dict = rpc.pcompile(player_color, player_latest)
    re_pattern = patterns_dict[rpc.SpecialPatterns.TASSHI]
    matched = re.findall(re_pattern, pattern)
    self.assertEqual(len(matched), match_nums)
  ...
  def test_yon(self, player_color: PlayerColor, player_latest: PlayerLatest, pattern: str, match_nums: int):
    patterns_dict = rpc.pcompile(player_color, player_latest)
    re_pattern = patterns_dict[rpc.SpecialPatterns.YON]
    matched = re.findall(re_pattern, pattern)
    self.assertEqual(len(matched), match_nums)
  ...
  def test_yonyon_ryoto(self, player_color: PlayerColor, player_latest: PlayerLatest, pattern: str, match_nums: int):
    patterns_dict = rpc.pcompile(player_color, player_latest)
    re_pattern = patterns_dict[rpc.SpecialPatterns.YONYON_RYOTO]
    matched = re.findall(re_pattern, pattern)
    self.assertEqual(len(matched), match_nums)
  ...
  def test_yonyon_tyoda(self, player_color: PlayerColor, player_latest: PlayerLatest, pattern: str, match_nums: int):
    patterns_dict = rpc.pcompile(player_color, player_latest)
    re_pattern = patterns_dict[rpc.SpecialPatterns.YONYON_TYODA]
    matched = re.findall(re_pattern, pattern)
    self.assertEqual(len(matched), match_nums)
  ...
  def test_yonyon_soryu(self, player_color: PlayerColor, player_latest: PlayerLatest, pattern: str, match_nums: int):
    patterns_dict = rpc.pcompile(player_color, player_latest)
    re_pattern = patterns_dict[rpc.SpecialPatterns.YONYON_SORYU]
    matched = re.findall(re_pattern, pattern)
    self.assertEqual(len(matched), match_nums)
  ...
  def test_san(self, player_color: PlayerColor, player_latest: PlayerLatest, pattern: str, match_nums: int):
    patterns_dict = rpc.pcompile(player_color, player_latest)
    re_pattern = patterns_dict[rpc.SpecialPatterns.SAN]
    matched = re.findall(re_pattern, pattern)
    self.assertEqual(len(matched), match_nums)